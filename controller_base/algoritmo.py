...
class AlgoritmoMTSP():
    ...
    def getCromosomaRandom (self):
        ...
        ciudades = [0]
        while 0 in ciudades or sum(ciudades) != len(self.nodos): # Asegurar que a todos los UAVs se le asigna al menos a un nodo y que se repartan todas las ciudades
    ...
    def getBestCromosoma (self): # TODO: buscar forma mejor
        ...
        '''
        return self.poblacion[list(self.poblacion[:,self.INDEX_FITNESS]).index(min(self.poblacion[:,self.INDEX_FITNESS]))] 
    ...
    def getRutasSubOptimas (self, n_generaciones = 5000, n_nearest_rr = 5, n_nearest = 5, n_random = 1000 , 
        ...
        for generacion in range(n_generaciones):
            ...
            if best_cromosoma is None or not np.array_equal(best_cromosoma, new_best_cromosoma):
                ...
                print('new_best_cromosoma:', new_best_cromosoma[self.INDEX_CROMOSOMA], 'fitness:', new_best_cromosoma[self.INDEX_FITNESS])
    ...
    def dibujarRutasFitnessCromosoma (self, cromosoma, fitness_cromosoma, show = True, ruta_guardar = None):
        ...
        '''
        # Dibujar nodos:
        grid = utils.dibujarNodos(self.nodos, aspect = max(np.array(list(self.nodos.values()))[:,0]) / max(np.array(list(self.nodos.values()))[:,1]))
    ...
    def asignarPosicionInicialGrafo (self):
        '''
        Asigna como posicion inicial de los drones el nodo mas cercano mediante "RoundRobin"
        '''
        vecinos = np.array([self.nbrs.kneighbors([dron.posicion_actual]) for dron in self.drones])
        distancias = vecinos[:,0]
        indices = vecinos[:,1]

        posiciones_nuevas = [None for i in range(len(self.drones))]
                ...
                if minimo in distancias[i] and posiciones_nuevas[i] is None:
                    for indice in indices[i][0]:
                        ...
                        if indice not in posiciones_ya_asignadas:
                            posiciones_nuevas[i] = self.nodos.get(indice)
                            dron.posicion_actual = self.nodos.get(indice)
